...
function initializeFirebase() {
    try {
        if (typeof firebase === 'undefined') {
            console.warn('❌ Firebase SDK não carregou');
            firebaseReady = false;
            return false;
        }

        if (typeof firebaseConfig === 'undefined') {
            console.warn('❌ firebaseConfig não definido');
            firebaseReady = false;
            return false;
        }
...
function updateConnectionStatus() {
    ...
    if (isOnline && firebaseReady) {
        ...
        indicator.title = 'Usando localStorage';
    } else {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (googleBtn) {
        googleBtn.addEventListener('click', async () => {
            if (!firebaseReady) {
                alert('❌ Firebase não disponível. Tente novamente.');
                return;
            }
            try {
                showLoading(true);
...
function saveData() {
    ...
    if (firebaseReady && currentUser && isOnline) {
        saveDataToFirebase();
    }
}
...
function renderEditCategoriesModal() {
    ...
    html += `<button class="btn btn-sm btn-success mb-3 add-category-btn" data-type="RECEITAS">+ Adicionar</button>`;
    ...
    html += `<button class="btn btn-sm btn-danger mb-3 add-category-btn" data-type="DESPESAS">+ Adicionar</button>`;
    ...
    html += `<button class="btn btn-sm btn-primary add-category-btn" data-type="CARTOES">+ Adicionar</button>`;
...
function renderPrevisao() {
    ...
    CATEGORIES.RECEITAS.forEach(cat => {
        ...
        html += `<div class="item-row d-flex justify-content-between align-items-center mb-2 pb-2 border-bottom border-dashed"><label for="previsao-${cat.id}" class="form-label mb-0">${cat.name}</label><input type="number" id="previsao-${cat.id}" data-id="${cat.id}" value="${val.toFixed(2)}" step="0.01" class="form-control w-auto text-end"></div>`;
    });
